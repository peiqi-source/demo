...
Y = Y(:);
    ...
    % =====================================================================
    % 【核心降维魔法】：用 sparse 瞬间生成混淆矩阵 G！(耗时 0.001 秒)
    % 彻底取代原本 MutualInfo、bestMap、rand_index 里那 3 坨 $C^2 \times N$ 的 find 循环！
    % G(i,j) 表示 真实标签为 i 且 预测标签为 j 的样本个数
    % =====================================================================
    ...
    % ---------------------------------------------------------
    maxClass = max(nClass1, nClass2);
    G_sq = zeros(maxClass);
    G_sq(1:nClass1, 1:nClass2) = G;
    [c, ~] = hungarian(-G_sq); 
    
    ACC = 0;
    for j = 1:length(c)
        % c(j) = i 表示：预测簇 j 对应于真实簇 i
        if c(j) > 0 && c(j) <= nClass1 && j <= nClass2
            ACC = ACC + G(c(j), j);
        end
    end
    ACC = ACC / n;
    ...
    % 【核弹拆除】：利用组合数从混淆矩阵中直接推导，彻底消灭 O(N^2) 样本对循环！
    % ---------------------------------------------------------
    % TP (True Positives): 真实为同类，预测也为同类的对数
    TP = sum(G(G > 1) .* (G(G > 1) - 1)) / 2;
    
    % TP + FP: 预测为同类的所有对数
    sum_col = sum(G, 1)';
    TP_FP = sum(sum_col(sum_col > 1) .* (sum_col(sum_col > 1) - 1)) / 2;
    
    % TP + FN: 真实为同类的所有对数
    sum_row = sum(G, 2);
    TP_FN = sum(sum_row(sum_row > 1) .* (sum_row(sum_row > 1) - 1)) / 2;
