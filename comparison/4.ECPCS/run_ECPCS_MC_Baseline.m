% =========================================================================
% ECPCS-MC 对比算法自动化测试脚本 (顶刊 Mean±Std 格式 + 5指标全面评估 + OOM防御)
...
t = 20;             % ECPCS-MC 内部随机游走步长 (论文默认 20)

% 确保 ECPCS-MC 需要的 Ncut 库在路径中
addpath('Ncut');    
...
csvFileName = sprintf('Baseline_ECPCS_MC_5Metrics_M%d_20Seeds_MeanStd.csv', M);
...
disp('====================================================================');
disp(['>>> 启动 ECPCS-MC 自动化基准测试 (带 Mean±Std 统计) | 选取 M = ', num2str(M)]);
...
for d_idx = 1:numDatasets
    ...
    try
        for runIdx = 1:numRuns
            ...
            % 运行 ECPCS-MC 算法并独立计时
            tic;
            Label = ECPCS_MC(selected_baseCls, K, t);
            run_time = toc;
