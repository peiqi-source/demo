...
csvFileName = fullfile(resultsDir, sprintf('grid_search_MSRA25_AnchorsNum_%s.csv', timestamp));
        ...
        for ns = param_num_sampling
            for k_val = param_k
                for seed = param_rng
                    ...
                    % 3. 构建单行表格
                    result_data = table(ar, ord, ns, k_val, seed, ACC, MIhat, Purity, Fscore, runtime, ...
                        'VariableNames', {'AnchorsRate', 'Order', 'NumSampling', 'K', 'Seed', 'ACC', 'NMI', 'Purity', 'Fscore', 'Runtime'});
                    % 4. 追加写入磁盘
                    if isFirst
