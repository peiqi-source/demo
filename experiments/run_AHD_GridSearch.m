...
%% 参数定义 (网格搜索空间)
param_anchors_rate = [20 22 24 28];
param_order = 1:6;
param_num_sampling = 1:6;
param_k = [2 3 4 5];
param_rng = 20:20:200;
param_delta = 5;

% 大规模数据集
% param_anchors_rate = [30 50];
% param_order = 2:5;
...
total_loops_per_dataset = length(param_anchors_rate) * length(param_order) * length(param_delta) * ...
...
for data_idx = dataset_list
    ...
    csvFileName = fullfile(resultsDir, sprintf('para_small_data%d_%s.csv', data_idx, timestamp));
    matFileName = fullfile(resultsDir, sprintf('para_small_data%d_%s.mat', data_idx, timestamp));
    
    % 预分配 .mat 的结构体空间
    empty_struct = struct('Anchors', [], 'Order', [], 'NumSampling', [], ...
        'K', [], 'Delta', [], 'Seed', [], 'ACC', [], 'NMI', [], 'Purity', [], 'Fscore', [], 'ARI', [],...
    ...
    result_cell = cell(total_loops_per_dataset, 19);
                ...
                for k_val = param_k
                    for delta = param_delta   
                        for seed = param_rng
                            ...
                            % 2. 评估聚类结果
                            [ACC, MIhat, Purity, Fscore, P, R, RI, ARI] = ClusteringMeasure4(Y, F);
                            ...
                            result_cell(loop_idx, :) = {data_idx, ar, anchors_str, ord, ns, k_val, delta, seed, ...
                                ACC, MIhat, Purity, Fscore, P, R, ARI, runtime, obj_str, alphaA_str};
                            ...
                            all_results_mat(loop_idx).Delta = delta;
                            ...
                            all_results_mat(loop_idx).ARI = ARI;
    ...
    fprintf('\n>>> 正在将数据集 %d 的指标导出至 CSV...\n', data_idx);
    varNames = {'DatasetID', 'AnchorsRate', 'Anchors', 'Order', 'NumSampling', 'K', 'Delta', 'Seed', ...
                'ACC', 'NMI', 'Purity', 'Fscore', 'P', 'R', 'ARI', 'Runtime', ...
