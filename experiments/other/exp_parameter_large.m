%% 性能指标提升实验 (彻底解决动态扩容与高频 I/O 写入慢的问题)
clear;
clc;
close all;
...
% 大规模数据集
param_anchors_rate = [20 60];
param_order = 2:6;
param_num_sampling = 2:6;
param_k = [5 7 10 15];
param_rng = 2:2:10;
delta = 20;
...
for data_idx = dataset_list
    ...
    csvFileName = fullfile(resultsDir, sprintf('para_large_data%d_%s.csv', data_idx, timestamp));
    matFileName = fullfile(resultsDir, sprintf('para_large_data%d_%s.mat', data_idx, timestamp));
    ...
    result_cell = cell(total_loops_per_dataset, 17);
            ...
            for ns = param_num_sampling
                for k_val = param_k
                    for seed = param_rng
                        ...
                        result_cell(loop_idx, :) = {data_idx, ar, anchors_str, ord, ns, k_val, seed, ...
    ...
    fprintf('\n>>> 正在将数据集 %d 的指标导出至 CSV...\n', data_idx);
    varNames = {'DatasetID', 'AnchorsRate', 'Anchors', 'Order', 'NumSampling', 'K', 'Seed', ...
