function [ACC, MIhat, Purity, Fscore, P, R, RI, ARI] = ClusteringMeasure4(Y, predY)
...
% 严格 O(N) 复杂度，无损精度，彻底剥离冗余的纯 MATLAB 匈牙利算法
    ...
    ExpectedIndex = (TP_FN * TP_FP) / Total_Pairs;
    MaxIndex = (TP_FN + TP_FP) / 2;
    
    if MaxIndex == ExpectedIndex
        ARI = 1; % 避免分母为0的情况
    else
        ARI = (TP - ExpectedIndex) / (MaxIndex - ExpectedIndex);
    end
end