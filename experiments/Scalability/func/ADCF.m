function [f_label, obj, runtime, alphaA] = ADCF(H, F_init)
tic;
NITR = 100; 
[num, c] = size(F_init); 
V = length(H); 
...
for iter = 1:NITR
    for it = 1:10
        ...
        for i = 1:num 
            ...
            ui = zeros(1, c);
            for v = 1:V
                ui = ui + alpha(v) * C{v}(h_v(i, v), :); % fkT S ei
            end
            
            % 增量计算与决策
            del = (fsf + 2 * ui + sii) ./ (ff + 1 + eps) - (fsf ./ (ff + eps)); % 对所有候选簇一起算
            f0_m = (fsf(m) - 2 * ui(m) + sii) / (ff(m) - 1 + eps);
            del(m) = fsf(m) / (ff(m) + eps) - f0_m; % 单独计算 k=m 的情况
    ...
    sii = sum(alpha);
    sum_S2 = alpha' * B1 * alpha;
    obj(iter+1) = sum_S2 - 2 * sum(fsf ./ (ff + eps)) + c; % 新目标函数值
