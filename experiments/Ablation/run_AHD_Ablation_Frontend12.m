...
detailsDir = fullfile(resultsDir, sprintf('AHD_Frontend_NULL_3_Ablation_details_%s', timestamp_global));
if ~exist(detailsDir, 'dir'), mkdir(detailsDir); end

masterCsvName = fullfile(resultsDir, sprintf('Master_AHD_Frontend_NULL_3_Ablation_%s.csv', timestamp_global));
...
for data_idx = dataset_list
    ...
    % 详细记录表：每一行对应一个数据集、一个变体、一次 seed 运行。
    detail_rows = cell(num_variants * num_runs, 24);
    all_runs = struct([]);
    row_ptr = 0;
    ...
    for v = 1:num_variants
        ...
        for run_idx = 1:num_runs
            ...
            seed = data_idx * 1000 + run_idx * 100;
            rng(seed, 'twister');
            ...
            detail_rows(row_ptr, :) = {data_idx, dataset_names{data_idx}, variant_tags{v}, seed, ...
                cur_use_HO, cur_use_MS, cur_use_AW, anchors_rate, opt_order, opt_ns, k_val, delta, ...
                runInfo.NumOriginalGraphs, runInfo.NumBaseClusterings, runInfo.NumRepeatedBaseClusterings, ...
                num, dim, c, ACC, NMI, ARI, runtime, status, errMsg};
            ...
            all_runs(row_ptr).AnchorsRate = anchors_rate;
            all_runs(row_ptr).Anchors = anchors_full;
            all_runs(row_ptr).Order = opt_order;
            all_runs(row_ptr).NumSampling = opt_ns;
            all_runs(row_ptr).K = k_val;
            all_runs(row_ptr).Delta = delta;
            ...
            all_runs(row_ptr).ClusterNumbers = runInfo.ClusterNumbers;
            all_runs(row_ptr).RepresentationSourceIndex = runInfo.RepresentationSourceIndex;
            ...
            all_runs(row_ptr).Status = status;
            all_runs(row_ptr).ErrorMessage = errMsg;
        end
        ...
        fprintf(fid_master, '%d,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n', ...
            data_idx, dataset_names{data_idx}, variant_tags{v}, cur_use_HO, cur_use_MS, cur_use_AW, ...
            anchors_rate, opt_order, opt_ns, k_val, delta, ...
            firstInfo.NumOriginalGraphs, firstInfo.NumBaseClusterings, firstInfo.NumRepeatedBaseClusterings, valid_runs, ...
            avg_metrics(1), std_metrics(1), max_metrics(1), ...
            avg_metrics(2), std_metrics(2), max_metrics(2), ...
            avg_metrics(3), std_metrics(3), max_metrics(3), ...
            avg_metrics(4), std_metrics(4), max_metrics(4));
    ...
    detailVarNames = {'DatasetID','DatasetName','Variant','Seed','use_HO','use_MS','use_AW', ...
                      ...
                      'n','d','c','ACC','NMI','ARI','Runtime','Status','ErrorMessage'};
    ...
    save(detailMatName, 'all_runs', 'detailTable', 'anchors_full', 'opt_order', 'opt_ns', ...
        'anchors_rate', 'k_val', 'delta', 'target_num_base', 'X', 'Y', 'c', '-v7.3');
